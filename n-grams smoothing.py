...
def laplace_smoothing(ngram_counts, unigram_counts, vocab_size, n):
    ...
    for ngram in ngram_counts:
        ...
        if n > 1:
            ...
            context_count = sum(unigram_counts.values())

        smoothed_probs[ngram] = (
...
text = (
    ...
    "this is another sample text with some different words"
)
...
smoothed_probs = laplace_smoothing(
    ngram_counts,
    unigram_counts,
    vocab_size,
    n
...
for ngram, prob in smoothed_probs.items():
    print(f"{ngram}: {prob:.6f}")unigram_counts = count_ngrams(unigrams_list)
# Build vocabulary
vocab = build_vocabulary(text)
vocab_size = len(vocab)
# Apply Laplace smoothing
